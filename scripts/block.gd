...
@onready var colider: Area2D = $Colider
	...
	if Global.build_mode == true and Global.placing == true and !is_in_group("already_placed"):
		...
		for dir in inputs.keys():
			if event.is_action_pressed(dir):
				move(dir)
		if Input.is_action_just_pressed("enter") and is_coliding == false:
...
func _on_colider_area_entered(area: Area2D) -> void:
	is_coliding = true
func _on_colider_area_exited(area: Area2D) -> void:
	is_coliding = false
func _on_colider_body_entered(body: Node2D) -> void:
	is_coliding = true
func _on_colider_body_exited(body: Node2D) -> void:
	is_coliding = false
